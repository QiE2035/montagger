...
- **Pairing**: manual, like monloader, and one-to-many - `start()` only restores stored credentials, the operator connects per monbooru address in the WebUI (`pairing.py`). One offer is sent, a poll thread waits for approval, and each completed pairing appends a `[[pairing]]` table (url/token/peer/created_at) to montagger.toml - there is no sidecar file anymore. API calls authenticate with the entry's token; inbound requests present the peer secret, which is unique per pairing, so `entry_for_peer` recovers the source instance (and its url) for relay routing. On 401/403 drop only that instance and never re-offer silently (`MonbooruAuthError`).
- Per-instance API clients: `app.state.client_for(url)` lazily builds one `MonbooruClient` per paired url (token lookup bound to that url). Tasks in the pipeline and the store carry a `source` (= the monbooru url) so identical ids across instances never collide - `tasks` is keyed on (source, image_id) since schema v2.
